...
class KafkaMessageProducer(Producer):
    def __init__(self):
        ...
        logger.info(
            f"Kafka producer initialized with servers: {config.kafka.bootstrap_servers}"
        )
    ...
    def produce(self, topic: str, message: Message) -> None:
        ...
        try:
            ...
            self.producer.flush()
            logger.debug(f"Message sent to topic: {topic}, type: {message.type}")
    ...
    def produce_batch(self, topic: str, messages: List[Message]) -> None:
        ...
        try:
            for message in messages:
                self.producer.send(topic, value=message.model_dump())
            self.producer.flush()
            logger.info(f"Batch of {len(messages)} messages sent to topic: {topic}")
        except Exception as e:
