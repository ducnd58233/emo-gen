...
class ICrawler(ABC):
    ...
    def __init__(self, max_workers: int = 4, batch_size: int = None):
    ...
    def crawl(self, url: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i in range(0, len(sources), self.batch_size):
                ...
                logger.info(
                    f"Processing batch {i//self.batch_size + 1} of {(len(sources)-1)//self.batch_size + 1}")

                # Step 3: Extract items from each source in the batch (parallel)
                batch_results = self.process_batch(batch)

                # Step 4: Process and save results
                if batch_results:
                    processed_results = self.post_process(batch_results)
