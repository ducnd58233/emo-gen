...
class MetadataStorageStage(BaseStage[DataFrame, DataFrame]):
    ...
    def _process_impl(self, input_df: DataFrame, context: PipelineContext) -> DataFrame:
        ...
        # Extract minimal data needed from DataFrame - only for emojis that were stored in MinIO
        emoji_data = input_df.select("id", "name", "source").collect()

        # Prepare data for database insert - strict filtering by successful MinIO uploads
        db_records = []
        for row in emoji_data:
            emoji_id = str(row["id"])

            # Skip if not in successfully stored MinIO IDs
            if emoji_id not in minio_successful_ids:
                continue

            # Skip if no storage path available
        ...
        if db_records:
            successful_ids, failed_ids = self._store_records_in_batches(
                db_records)
            logger.info(
        ...
        # Only emojis that succeeded in both MinIO and database should be marked as processed
        final_successful_ids = [
            id for id in successful_ids if id in minio_successful_ids]
        context.set("successful_emoji_ids", final_successful_ids)

        return input_df

    def _store_records_in_batches(self, db_records: List[Dict]) -> Tuple[List[str], List[str]]:
    ...
    def _handle_error(self, error: Exception, input_data: DataFrame, context: PipelineContext) -> None:
        ...
        # Log detailed error information and update context
        logger.error(f"Error in metadata storage stage: {error}")

        # Set empty lists for downstream stages to handle properly
        context.set("metadata_stored_emoji_ids", [])
        context.set("metadata_failed_emoji_ids",
                    context.get("minio_stored_emoji_ids", []))

        # No emojis should be considered fully successful if metadata stage failed
        context.set("successful_emoji_ids", [])
